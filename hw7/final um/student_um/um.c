...
void um_execute(UM_T *um) {
        ...
        /* Get a pointer to segment zero */

        // Row_T seg_zero = um->mem->segments[0];
        // for (uint32_t i = 0; i < seg_zero.numWords; i++) {
        // sds[i];eg_zero.words[i] = um->mem->segments[0].wor

        // }

        uint32_t seg0len = um->mem->segments[0].numWords;

        uint32_t prog_counter = 0;
        uint32_t opcode, ra, rb, rc, word;

        /* Execute words in segment zero until there are none left */
        while (prog_counter < seg0len) {
                // printf("%d\n", prog_counter);
                /* Get the next word from segment zero */
                word = um->mem->segments[0].words[prog_counter];
                prog_counter++;
                ...
                case 12: /* Load Program */
                        ...
                        /* Get a pointer to the updated segment zero */
                        // seg_zero = um->mem->segments[0];
                        assert(um->mem->segments[0].words != NULL);
                        seg0len = um->mem->segments[0].numWords;
                        break;
                case 13: /* Load value */
                        {
...
void printRegisters(UM_T m) 
{
        for (int i = 0; i < 8; i++) {
                fprintf(stderr, "reg: %d val: %d\n", i, m.registers[i]);
        }
}
...
uint32_t load_program(UM_T *um, uint32_t ra, uint32_t rb, uint32_t rc)
{
    ...
    Row_T to_copy = um->mem->segments[rb_val];
    assert(to_copy.words != NULL);

    /* Creating a copy with the same specifications */
    uint32_t seg_len = to_copy.numWords;
    Row_T *copy = (Row_T *)malloc(sizeof(Row_T));
    copy->words = (uint32_t *)malloc(seg_len * sizeof(uint32_t));
    assert(copy->words != NULL);
    copy->numWords = seg_len;

    /* Deep copying */
    for (uint32_t i = 0; i < seg_len; i++) {
        copy->words[i] = to_copy.words[i];
    }

    /* Freeing segment 0 and inserting the copy */
    free(um->mem->segments[0].words);
    um->mem->segments[0] = *copy;
